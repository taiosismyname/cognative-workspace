...
- [x] Inspect the attached Sanct archive safely without executing its contents
- [x] Compare Sanct memory, model-interface, import, identity, and orchestration boundaries with Cognitive Workspace
- [x] Assess whether Sanct should merge into Cognitive Workspace, act as a bridge, or remain separate
- [x] Identify concrete reusable components and incompatibilities
- [x] Deliver a grounded comparison and recommendation without modifying either application


## Improved archive comparison

- [ ] Inventory `cognitive-workspace-fixed(1).zip` without executing its contents
- [ ] Compare its source and configuration against the active Cognitive Workspace project
- [ ] Verify fixes, tests, and continuity-relevant boundaries
- [ ] Determine whether the existing OpenRouter configuration is sufficient for live verification
- [ ] Report whether the uploaded archive should become the working baseline


## Verification and packaging

- [x] Verify the uploaded archive contains the improved OpenRouter adapter tests and continuity pipeline coverage
- [x] Merge or restore the improved continuity files into the active managed project if approved
- [x] Run type checking and the full test suite in the active project; run type checking and the production build in the uploaded archive
- [x] Run type checking, the full test suite, and the production build in the active managed project after any approved merge
- [x] Record the distinction between mocked OpenRouter contract tests and live-key verification
- [x] Save the packaged checkpoint of the verified active state after the merge decision


## Approved merge and release

- [x] Merge the verified improved archive into the active managed project
- [x] Add `providerConnectionId` to `messageOrigins` and update its schema index definition
- [x] Generate the migration with Drizzle and inspect it for exactly one add, one drop, and one add operation
- [x] Apply the reviewed migration to the database
- [x] Run active-project type check, full tests, and production build
- [x] Save the verified checkpoint
- [ ] Push the verified state to the actual GitHub repository


## GitHub synchronization target

- [ ] Verify `https://github.com/taiosismyname/cognative-workspace` in the authenticated browser
- [ ] Compare the target repository with checkpoint `b7b67293`
- [ ] Obtain confirmation immediately before any repository write
- [ ] Publish the verified project to the target repository
- [ ] Verify the published repository contents and commit


## Integrity audit before GitHub publication

- [x] Pause all GitHub writes until active project integrity is verified
- [x] Confirm active `server/db.ts` exports the functions used by `server/routers.ts`
- [x] Audit whether `Home.tsx` contains the continuity surface and whether `App.tsx` routes `/continuity`; verified that neither is present in the active project or checkpoint b7b67293
- [x] Compare active project files with checkpoint `b7b67293` and the uploaded archive
- [x] Run safe check/test/build without publishing
- [x] Repair or roll back only if a regression is verified


## Continuity UI integration

- [x] Add a real `/continuity` route backed by the existing continuity/state APIs
- [x] Show current model/provider and selected continuity identity
- [x] Show stored external state and published/current status
- [x] Show state/version history without adding persistence
- [x] Show imported conversations and derived memories/artifacts with provenance
- [x] Show the context that can actually be reconstructed from current backend data
- [x] Run type check, tests, and production build after the UI addition
- [x] Browser-test the real `/continuity` page and save a pre-GitHub checkpoint


## Continuity verification gaps

- [x] Add a model/continuity lane selector bound to real model and snapshot data
- [x] Expose imported conversations, continuity artifacts, and artifact-source provenance through the existing continuity read model
- [x] Show the exact persisted reconstructable context/evidence for the selected model lane
- [x] Save a new checkpoint after these continuity fixes and visual verification


## Final continuity evidence gap

- [x] Render artifact-source provenance details including conversation, message, quote, and relevance fields
- [x] Render the exact persisted reconstructable context/evidence payload for the selected model lane
