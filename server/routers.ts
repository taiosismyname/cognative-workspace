...
import { getDb, getCouncilResults, listConversations, listImportedConversations, listMemories, listMessages, listModels, listCouncilRuns, listProviderConnections, listHistoryImports, listStateArtifacts, listArtifactSources, listStateSnapshots, getOwnedSnapshot, getOrCreateProviderConnection, createHistoryImport, completeHistoryImport, findExistingConversationByNativeId, findExistingMessageNativeIds, insertImportedConversation, insertImportedMessages, ownsConversation, ownsModel } from "./db";
...
export const appRouter = router({
  ...
  continuity: router({
    ...
    snapshot: protectedProcedure.input(z.object({ id: z.number().int().positive() })).query(async ({ ctx, input }) => {
      const snapshot = await getOwnedSnapshot(ctx.user.id, input.id);
      if (!snapshot) throw new TRPCError({ code: "NOT_FOUND", message: "State snapshot not found." });
      const artifacts = await listStateArtifacts(snapshot.id);
      const sources = await listArtifactSources(artifacts.map(artifact => artifact.id));
      return { snapshot, artifacts, sources };
    }),
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        try {
          for (const conv of parsed) {
            ...
            messagesImported += await insertImportedMessages(conversationId, historyImport.id, connection.id, newMessages);
          }
